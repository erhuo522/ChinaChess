...
int test_chars_segment() {
  ...
  cv::Mat src = cv::imread("../../data/resources/image/chars_segment.jpg");
...
int test_chars_identify() {
  ...
  cv::Mat plate = cv::imread("../../data/resources/image/chars_identify.jpg");
  ...
  if (result == 0) {
	  for(auto iter = matChars.begin();iter != matChars.end(); ++iter)
      {
        auto character = CharsIdentify::instance()->identify(*iter);
...
int test_chars_recognise() {
  ...
  cv::Mat src = cv::imread("../../data/resources/image/chars_recognise.jpg");
  CCharsRecognise cr;
